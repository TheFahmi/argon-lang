...
*   **What it is:** The compiler logic written **entirely in Cryo language** (`self-host/compiler.cryo`).
*   **How it runs:** In the benchmark, this Cryo script is executed by the **Cryo Interpreter**. It reads a source file (e.g., `fib_poly.cryo`) and outputs LLVM IR.
...
*   **How it runs:** 
    1. `compiler.cryo` (running on interpreter) reads `fib_poly.cryo` -> generates `fib_poly.cryo.ll`.
    2. `clang` compiles `fib_poly.cryo.ll` -> `fib_poly_self` (Executable).
...
| :--- | :--- | :--- | :--- |
| **Cryo Interpreter** | Rust Program | **Runs Code** | 🐢 Slow (4.9s) |
| **Cryo Native (LLVM)** | CPU (Native) | **Runs Code** | ⚡ Fast (0.006s) |
| **Cryo Compiler (Self)** | Cryo Interpreter | **Compiles Code** | ⏱️ Compile Time (0.02s) |
| **Cryo Self-Hosted** | CPU (Native) | **Runs Code** | ⚡ Fast (0.006s) |
