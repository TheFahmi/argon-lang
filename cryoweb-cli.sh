#!/bin/bash
# ============================================
# CryoWeb Project Generator
# Usage: ./cryoweb-cli.sh new <project-name>
# ============================================

VERSION="1.0.0"
CRYO_BIN="${CRYO_BIN:-cryo}"  # Use 'cryo' or set CRYO_BIN env var
...
create_project() {
    ...
    if [ -z "$PROJECT_NAME" ]; then
        ...
        echo "Usage: cryoweb new <project-name>"
        exit 1
    fi
    
    echo ""
    echo "🚀 Creating new CryoWeb project: $PROJECT_NAME"
    echo ""
    ...
    # Create config
    cat > "$PROJECT_NAME/src/config/app.config.cryo" << 'EOF'
// Application Configuration
let APP_NAME = "CryoWeb API";
...
EOF
    ...
    # Create utils
    cat > "$PROJECT_NAME/src/common/utils/response.util.cryo" << 'EOF'
// Response Utilities
fn response_ok(data) {
    return json_object([
...
EOF
    ...
    # Create middleware
    cat > "$PROJECT_NAME/src/common/middleware/logger.middleware.cryo" << 'EOF'
// Logger Middleware
...
EOF
    ...
    # Create auth guard
    cat > "$PROJECT_NAME/src/common/guards/auth.guard.cryo" << 'EOF'
// Auth Guard
...
EOF
    ...
    # Create user entity
    cat > "$PROJECT_NAME/src/modules/users/user.entity.cryo" << 'EOF'
// User Entity
...
EOF
    ...
    cat > "$PROJECT_NAME/src/modules/users/users.service.cryo" << 'EOF'
// Users Service
let users_db = [];
let next_user_id = 1;
...
EOF
    ...
    cat > "$PROJECT_NAME/src/modules/users/users.controller.cryo" << 'EOF'
// Users Controller
...
EOF
    ...
    # Create auth service
    cat > "$PROJECT_NAME/src/modules/auth/auth.service.cryo" << 'EOF'
// Auth Service
...
EOF
    ...
    cat > "$PROJECT_NAME/src/modules/auth/auth.controller.cryo" << 'EOF'
// Auth Controller
fn handle_login(ctx) {
...
EOF
    ...
    cat > "$PROJECT_NAME/src/app.module.cryo" << 'EOF'
...
fn handle_root(ctx) {
    ctx.html("<h1>Welcome to CryoWeb</h1><p>Visit <a href='/api'>/api</a></p>");
}
...
EOF
    ...
    # Create main.ar
    cat > "$PROJECT_NAME/src/main.cryo" << 'EOF'
// ============================================
// CryoWeb Application
// Generated by CryoWeb CLI
// ============================================
...
EOF
    ...
    echo "  cryo src/main.cryo"
    echo ""
    echo "Or if cryo is not in PATH:"
    echo "  ../cryo.exe src/main.cryo"
    echo ""
}
...
case "$1" in
    "new")
        ...
        ;;
    "version"|"-v"|"--version")
        echo "CryoWeb CLI v$VERSION"
        ;;
    "help"|"-h"|"--help"|"")
        echo "CryoWeb CLI v$VERSION"
        echo ""
        echo "Usage:"
        echo "  cryoweb new <project-name>   Create new project"
        echo "  cryoweb version              Show version"
        echo "  cryoweb help                 Show help"
        echo ""
        ;;
    *)
        echo "Unknown command: $1"
        echo "Run 'cryoweb help' for usage"
        exit 1
        ;;
esac
